...
export function CourseCard({
  ...
  return (
    <Card
      className={cn(
        "relative w-60 mb-4 border-l-4 shadow-sm",
        statusColor
      )}
    >
      <CardContent className="p-3 flex flex-col gap-1">
        ...
        <div className="absolute top-2 right-2 flex gap-1">
          {status === "bloqueado" && <Lock className="h-4 w-4 text-muted-foreground" />}
          {prereqsCount > 0 && <Info className="h-4 w-4 text-muted-foreground" />}
        </div>
      </CardContent>
    </Card>
  )
